...
class NurbsCurve:
    ...
    def compute_nurbs_at(self, u):
    
        #create homogenous points
        self.hPoints=[]
        for w,i in enumerate(self.weights):
            self.hPoints.append([self.points[i][0]*w,w])

        degree=2 # for now
        order=degree+1
        nbknots=len(self.points)+degree+1
        #if multiplicyt_touch_first_point=degree+1 then it touch
        


GetPoint_DeBoor(k, u)
    s = 0
    while (k >= s && knots[k - s] == u)
        s++
    Q = new ControlPoint[p - s + 1]
    for (i = k - p; i <= k - s; i++)
        Q[i - k + p] = P[i].ConverTo4D()

    for (r = 1; r <= p - s; r++)
        for (i = k - s; i >= k - p + r; i--)
            a = (u - self.knots[i]) / (self.knots[i + p - r + 1] - self.knots[i])
            j = i - k + p
            Q[j] = (1 - a) * Q[j - 1] + a * Q[j]

    return Q[p-s].ConvertTo3D().To3D()
...
def create_quarter_circle_nurbs():
    # from https://www.geometrictools.com/Documentation/NURBSCircleSphere.pdf, chapter 2.1.1
    n=NurbsCurve
    n.points=[ [1, 0] ,[1, 1],[0, 1] ] #deg 2 because 2 points
    n.weights=[1, 1, 2]
    n.knots=[0,0,0,1,1,1]
    return n
########################################################################
...
#test nurbs
nurbs=create_quarter_circle_nurbs()
x=[]
y=[]
for t in np.arange(0,1,0.01):
    p=nurbs.compute_nurbs_at(t)
    x.append[p.x]
    y.append[p.y]
plt.plot(x,y,label='nurbs')
norm=np.sqrt(x**2+y**2)
print(f"nurbs max_norm={norm.max()} min_norm={norm.min()}")
