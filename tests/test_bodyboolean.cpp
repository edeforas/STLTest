...
int main()
{
	...
	Mesh meshUnion;
	bu.to_mesh(meshUnion);
	if (!OBJFile::save("two_spheres_union.obj", meshUnion))
	{
	...
	Mesh meshInter;
	bi.to_mesh(meshInter);
	if (!OBJFile::save("two_spheres_intersection.obj", meshInter))
	{
	...
	Mesh meshDiff;
	bd.to_mesh(meshDiff);
	if (!OBJFile::save("two_spheres_difference.obj", meshDiff))
	{
